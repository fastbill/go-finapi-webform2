...
func (v *WebFormType) UnmarshalJSON(src []byte) error {
	...
	enumTypeValue := WebFormType(value)
	for _, existing := range AllowedWebFormTypeEnumValues {
	...
	return fmt.Errorf("%+v is not a valid WebFormType", value)
}

// NewWebFormTypeFromValue returns a pointer to a valid WebFormType
...
func NewWebFormTypeFromValue(v string) (*WebFormType, error) {
	...
	if ev.IsValid() {
		...
		return nil, fmt.Errorf("invalid value '%v' for WebFormType: valid values are %v", v, AllowedWebFormTypeEnumValues)
	}
}
...
func (v NullableWebFormType) Get() *WebFormType {
	return v.value
}
...
func NewNullableWebFormType(val *WebFormType) *NullableWebFormType {
	return &NullableWebFormType{value: val, isSet: true}
}
